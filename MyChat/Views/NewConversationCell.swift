...
//  MyChat
//
//  Created by Aditya Ambekar on 05/08/20.
...
class NewConversationCell: UITableViewCell {
    ...
    static let indentifier = "NewConversationCell "
    ...
    public func configure(with model: SearchResult) {
        ...
        StorageManager.shared.downloadURLForPath(for: path) {[weak self] (result) in
            ...
            case .success(let url):
                //dont need to download image again and again
                self?.userImageView.sd_setImage(with: url,
                                                completed: nil)
            case .failure(let error):
                print("Failed to get Imgae \(error)")
            }
        }
